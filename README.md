# 👋 Hi, I'm Nicolas Tosawa

[🇺🇸 English](#-english) • [🇧🇷 Português](#-português) • [🇯🇵 日本語](#-日本語)

---

## 🇺🇸 English

🎓 I'm an **Analysis and Systems Development** student in Brazil.

🎮 I'm especially interested in **Game Development**, while building a strong foundation in software development and exploring different areas of programming.

🌎 Native Portuguese speaker and fluent in English.

### 🚀 Currently learning

- Programming logic and software development
- C++
- Game Development
- Web Development
- Databases

### 📌 Featured Projects

#### ⚔️ [Jujutsu Fighting](https://github.com/wdninjaytb/JUJUTSU-FIGHTING)
A 2D fighting game developed with **Construct 3** as an academic project, inspired by *Jujutsu Kaisen*.

#### 🐸 Kaeru Produtos Orientais

Academic web project simulating an oriental products store, divided into two applications:

- [**Public Catalog**](https://github.com/wdninjaytb/kaeru-catalogo) — Public interface for browsing products and categories.
- [**Admin Panel**](https://github.com/wdninjaytb/kaeru-adm) — Administrative system for managing products, categories and inventory, with a built-in dashboard for monitoring inventory and system data.

### 🛠️ Technologies I've worked with

#### 💻 Programming Languages
`C++` `Java` `Lua / Luau` `PHP` `JavaScript` `TypeScript`

#### 🌐 Web
`HTML` `CSS / SCSS` `Bootstrap`

#### 🗄️ Databases
`MySQL` `MariaDB`

#### 🎮 Game Development
`Construct 3`

---

## 🇧🇷 Português

🎓 Sou estudante de **Análise e Desenvolvimento de Sistemas** no Brasil.

🎮 Tenho interesse principal em **Game Development**, enquanto desenvolvo minha base em desenvolvimento de software e exploro diferentes áreas da programação.

🌎 Português nativo e inglês fluente.

### 🚀 Atualmente estudando

- Lógica de programação e desenvolvimento de software
- C++
- Desenvolvimento de jogos
- Desenvolvimento Web
- Banco de dados

### 📌 Projetos em destaque

#### ⚔️ [Jujutsu Fighting](https://github.com/wdninjaytb/JUJUTSU-FIGHTING)
Jogo de luta 2D desenvolvido no **Construct 3** como projeto acadêmico, inspirado em *Jujutsu Kaisen*.

#### 🐸 Kaeru Produtos Orientais
Projeto acadêmico de desenvolvimento web que simula uma loja de produtos orientais, com catálogo público e sistema administrativo separado.

- [**Catálogo Público**](https://github.com/wdninjaytb/kaeru-catalogo) — Interface pública de catálogo de produtos e categorias.
- [**Painel Administrativo**](https://github.com/wdninjaytb/kaeru-adm) — Sistema administrativo para gerenciamento de produtos, categorias e estoque, com um dashboard integrado para monitoramento do estoque e de informações importantes do sistema.

### 🛠️ Tecnologias com as quais já trabalhei

#### 💻 Linguagens de Programação
`C++` `Java` `Lua / Luau` `PHP` `JavaScript` `TypeScript`

#### 🌐 Web
`HTML` `CSS / SCSS` `Bootstrap`

#### 🗄️ Bancos de Dados
`MySQL` `MariaDB`

#### 🎮 Desenvolvimento de Jogos
`Construct 3`

---

## 🇯🇵 日本語

🎓 ブラジルで**システム分析・開発（ADS）**を学んでいる学生です。

🎮 **ゲーム開発**に特に興味があり、ソフトウェア開発の基礎を学びながら、さまざまなプログラミング分野について勉強しています。

🌎 母語はポルトガル語で、英語も話せます。現在、日本語を勉強しています。

### 🚀 現在勉強していること

- プログラミングとソフトウェア開発の基礎
- C++
- ゲーム開発
- Web開発
- データベース

### 📌 主なプロジェクト

#### ⚔️ [Jujutsu Fighting](https://github.com/wdninjaytb/JUJUTSU-FIGHTING)
**Construct 3**を使用して制作した、*呪術廻戦*にインスパイアされた2D格闘ゲームの大学プロジェクトです。

#### 🐸 Kaeru Produtos Orientais

東洋の商品を扱う店舗を想定して制作した大学のWeb開発プロジェクトです。2つのアプリケーションに分かれています。

- [**商品カタログ**](https://github.com/wdninjaytb/kaeru-catalogo) — 商品やカテゴリーを閲覧するための公開ページです。
- [**管理システム**](https://github.com/wdninjaytb/kaeru-adm) — 商品、カテゴリー、在庫を管理するためのシステムです。在庫やシステムの情報を確認できるダッシュボードも搭載しています。

### 🛠️ 使用経験のある技術

#### 💻 プログラミング言語
`C++` `Java` `Lua / Luau` `PHP` `JavaScript` `TypeScript`

#### 🌐 Web
`HTML` `CSS / SCSS` `Bootstrap`

#### 🗄️ データベース
`MySQL` `MariaDB`

#### 🎮 ゲーム開発
`Construct 3`

---

## 📫 Contact / Contato / 連絡先

[LinkedIn](https://www.linkedin.com/in/nicolas-tosawa-1ba67b2b0/)
